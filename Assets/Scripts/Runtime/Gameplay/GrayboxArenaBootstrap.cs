...
namespace Caineng.Playground.Gameplay
{
    public sealed class GrayboxArenaBootstrap : MonoBehaviour
    {
        private void Awake()
        {
            ...
            CreateBlock(root.transform, "Cyan Platform", new Vector3(-5f, 0.35f, 3f), new Vector3(5f, 0.7f, 4f), new Color(0.1f, 0.85f, 1f), SurfaceKind.Liquid);
            CreateBlock(root.transform, "Magenta Platform", new Vector3(5f, 1.1f, 5f), new Vector3(4f, 0.7f, 4f), new Color(1f, 0.15f, 0.65f), SurfaceKind.Float);
            CreateBlock(root.transform, "Yellow Ramp", new Vector3(0f, 0.8f, 7f), new Vector3(4f, 0.5f, 6f), new Color(1f, 0.75f, 0.1f), SurfaceKind.Bounce, new Vector3(15f, 0f, 0f));
            ...
            var respawn = player != null ? player.GetComponent<RespawnController>() : null;
            if (respawn != null) respawn.Configure(new Vector3(0f, 1f, 0f));
        }

        private static void CreateBlock(Transform parent, string name, Vector3 position, Vector3 scale, Color color, SurfaceKind surfaceKind = SurfaceKind.None, Vector3? rotation = null)
        {
            ...
            if (surfaceKind != SurfaceKind.None) block.AddComponent<TraversalSurface>().Configure(surfaceKind);
        }
    }
}
